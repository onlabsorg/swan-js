    ...
    compare (other, _compareSameTypeValues) {
        if (other instanceof this.constructor) {
            if (typeof _compareSameTypeValues === 'function') {
                ...
                return this.unwrap() === other.unwrap() ? 0 : NaN;
            }
        } else {
            return other.isNothing() ? +1 : NaN;
        }
    }
}
    ...
    *items () {
    ...
    compare (other) {
        for (let [item1, item2] of this.iterPairs(other)) {
            if (item1.isNothing()) return item2.isNothing() ? 0 : -1;
            if (item2.isNothing()) return +1;
...
class Bool extends Item {
    ...
    compare (other) {
        return super.compare(other, (thisIsTrue, otherIsTrue) => {
            ...
            if (thisIsFalse && otherIsTrue) return -1;
            if (thisIsTrue && otherIsFalse) return +1;
            return 0;
        })
    }
}
...
class Text extends Sequence {
    ...
    compare (other) {
        return super.compare(other, (thisString, otherString) => {
            return thisString.localeCompare(otherString);
        });
    }
}
