...
T.defineBinaryOperation = handlers => async function (val1, val2) {
    ...
    return await handlerFn.call(this, val1, val2);
}
...
T.defineFunction = handlers => async function (...args) {
    const x = args.length > 1 ? T.createTuple(...args) : args[0]
    const xType = T.detectType(x);
    const f = handlers[xType] || handlers["Anything"];
    return await f.call(this, x);
}
...
T.Anything  = {};
T.Nothing   = { __prototype__: T.Anything };
T.Boolean   = { __prototype__: T.Anything };
T.Number    = { __prototype__: T.Anything };
T.Sequence  = { __prototype__: T.Anything };
T.String    = { __prototype__: T.Sequence };
T.List      = { __prototype__: T.Sequence };
T.Function  = { __prototype__: T.Anything };
T.Namespace = { __prototype__: T.Anything };
T.Tuple     = { __prototype__: T.Anything };

T.getPrototypeOf = val => {
    const typeName = T.detectType(val);
    return (typeName === 'Namespace' && T.isNamespace(val.__prototype__)) ?
            val.__prototype__ : T[typeName];
}

T.resolveName = (val, name) => {
    if (T.isNamespace(val) && val[name] !== undefined) return val[name];
    if (val === T.Anything) return T.NOTHING;
    const prototype = T.getPrototypeOf(val);
    return T.resolveName(prototype, name);
}
