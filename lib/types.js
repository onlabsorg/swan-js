...
class Namespace extends Mapping {
    ...
    // Assigns a set of values (the term2 tuple) to a set of identifiers (the
    // term1 tuple). If term1 has more identifiers than the values in term2,
    // all the identifiers in excess will be assigned an empty tuple. If term1
    // has less identifiers than the values in term2, then the last identifier
    // will be mapped to the tuple of the values in excess.
    assign (term1, term2) {
    ...
    sum (other) {
        const obj = Object.create( this.unwrap() );
        return new this.constructor( Object.assign(obj, other.unwrap()) );
    }
...
class Func extends Item {
    ...
    async apply (...items) {
        return wrap( await this.unwrap()(...items.map(unwrap)) );
    }
