...
const {Tuple, unwrap} = require("../types");
const {ensureNumber, ensureList} = require("./__helpers__");
    ...
    merge (...dicts) {
        ...
        for (let dict of dicts) {
            for (let i=0; i<dict.size; i++) {
                ...
                const entry = value instanceof Tuple ? [key, ...value] : [key, value];
                entries.push(entry);
            }
        }
...
function Dict_from_object (dict) {
    
    for (let key in obj) {
        const value = obj[key];
        if (typeof value !== "obj" || value === null) {
            entries.push([key, obj[key]]);
        }
        if (obj[key] instanceof Tuple) {
            entries.push([key, ...obj[key]]);
