...
const builtins = module.exports = {
    ...
    async str (...values) {
        ...
        const textTuple = await term.imapAsync(async (item) => {
            if (item instanceof types.Namespace) {
                ...
                if (__str__ instanceof types.Text) return __str__;
            }
            return item;
        });
    ...
    Numb: {
         ...
         */
        parse (...values) {
            const textTuple = new types.Tuple(...values);
            const numbTuple = textTuple.vmapSync(
                    value => typeof value === "string" ? Number(value) : NaN);
            return numbTuple.normalize();
        }
    }
}
