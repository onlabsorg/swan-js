...
module.exports = types => {
     ...
     *  
     *  If the argument of `fi` is not a Text item, `fi` returns Undefined Number.
     *  
     *  If the parameter of `fi` is a tuple, it applies to each item of the
     *  tuple and returns a tuple.
     */
    text.find = SyncMap(s1 => AsyncMap(s2 => isString(s1) && isString(s2) ? s2.indexOf(s1) : NaN));
     ...
     *  tuple and returns a tuple.
     */
    text.rfind = SyncMap(s1 => AsyncMap(s2 => isString(s1) && isString(s2) ? s2.lastIndexOf(s1) : NaN));
     ...
     *  character position is computed as relative to the end of `L`.
     *  
