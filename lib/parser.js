...
class Parser {
    ...
    constructor (options) {
        ...
        this.lexer = new Lexer({binaryOperators: Object.keys(options.binaryOperations)});
    }
    ...
    parseExpression (tokens, done) {
        ...
        while (true) {
            ...
            if (tokens.get() && tokens.get().matchBinaryOperator()) {
                ...
                expression.push(operator); tokens.inc();
                if (done()) {
                ...
                // operator.
                let operator = this.options.binaryOperations[""];
    ...
    parseOperand (tokens) {
        ...
        if (tokens.get() && tokens.get().matchSign() && tokens.get(+1).matchNumberLiteral()) {
            let factor = tokens.get().value === "-" ? -1 : 1;
            operand = this.createOperation(this.options.numberHandler, [tokens.get(+1).value * factor], tokens.get(+1).position); tokens.inc(2);
        }
        
        // If just a number literal
        else if (tokens.get() && tokens.get().matchNumberLiteral()) {
