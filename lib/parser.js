...
const Parser = module.exports = (options) => {
    ...
    const lexer = new Lexer({binaryOperators: Object.keys(options.binaryOperations)});

    // parse function: expression source -> evaluate function`
    return source => {
        
        const tailToken = {position: new Lexer.Position(source, source.length)};
        ...
        const parseExpression = (done) => {
            ...
            if (done()) {
                const lastToken = tokens[i] || tokens[tokens.length-1] || tailToken;
            ...
            while (true) {
                ...
                if (tokens[i] && tokens[i].matchBinaryOperator()) {
                    ...
                    if (done()) {
                        throw SyntaxError('Operand expected', tokens[i] || tailToken);
                    }
        ...
        try {
            ...
            var tokens = Array.from(lexer.tokenize(source));    // array of tokens
