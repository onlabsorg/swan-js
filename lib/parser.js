...
const Parser = module.exports = (options) => {
    ...
    return source => {
        ...
        const SyntaxError = (token, message) => token ? 
                new Lexer.SyntaxError(token.position.source, token.position.position, message) :
                new Lexer.SyntaxError(source, source.length, message);
        ...
        const createOperation = (handlerName, operands, position) => async context => {
            try {
                ...
                return await context[handlerName](...operands);
                
            } catch (error) {
                return await context[options.errorHandler](error);
            }
        }
        ...
        const parseExpression = (done) => {
            ...
            while (true) {
                ...
                if (tokens[i] && tokens[i].matchBinaryOperator()) {
                    ...
                    if (done()) {
                        throw SyntaxError(tokens[i], 'Operand expected');
                    }
