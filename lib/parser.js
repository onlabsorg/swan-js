...
// The main export of this module creates and returns `parse` function,
...
module.exports = (options) => {
    ...
    return (source) => {
        ...
        const parseExpression = (done) => {
            ...
            while (true) {
                ...
                if (tokens[i] && tokens[i].matchBinaryOperator()) {
                    let operator = wrapOperator(tokens[i]); i++;
        ...
        const createOperation = (handlerName, operands, position) => async scope => {
            if (typeof scope[handlerName] !== "function") throw new Error(`'${handlerName}' handler not defined in this context`);
            
            scope.$$parser.stack.push(position);
            const result = await scope[handlerName](...operands);
            scope.$$parser.stack.pop();
        ...
        return scope => {
            scope.$$parser = {
                stack: [],
                getCurrentLocation () {
                    if (this.stack.length === 0) return {line:"", row:0, column:0};
                    position = this.stack[this.stack.length-1];
                    let row=0, column=position; 
                    while (column > lines[row].length) {
                        column = column - lines[row].length - 1;
                        row = row + 1;
                    }
