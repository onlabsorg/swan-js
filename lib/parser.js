...
module.exports = (options) => {
    ...
    return source => {
        ...
        const tokens = tokenize(source);    // array of tokens
        var i = 0;                          // index of the next token in line


        // The `createOperation` funcction returns a function that, once called
        // with a `scope` object, executes an operation and return its value.
        ...
        const createOperation = (handlerName, operands, position) => async scope => {
            ...
            try {
                ...
                result = await scope[handlerName](...operands);
                
            } catch (error) {
                result = await scope[options.errorHandler](error);
            }
        ...
        // Parse the entier expression
        const rootOperation = parseExpression( () => i >= tokens.length );

        // Return the `evaluate` function`
        return scope => {
            
            // Attach to the scope a `$$parser` object that the scope functions
            // can use to retrieve the source location of the operation 
            ...
            scope.$$parser = {
                ...
                getCurrentLocation () {
                    ...
                    const position = this.stack[this.stack.length-1];
