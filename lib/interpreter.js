...
class BinaryOperation extends ASTNode {
    ...
    undefined (leftHandOperand, rightHandOperand) {
        return new types.Undefined(this.position, this.constructor.name, null, leftHandOperand, rightHandOperand)
    }
...
class Leaf extends ASTNode {
    ...
    undefined (value) {
        return new types.Undefined(this.position, this.constructor.name, types.wrap(value));
    }
...
class ArithmeticOperation extends BinaryOperation {
    ...
    async evaluate (context) {
        ...
        return fn => pairsTuple.imapSync(pair => {
            const item1 = pair.get(0), value1 = item1.unwrap(), type1 = item1.typeName;
            const item2 = pair.get(1), value2 = item2.unwrap(), type2 = item2.typeName;
            return fn(item1, item2, value1, value2, type1, type2);
        });
    }
}
...
class SumOperation extends ArithmeticOperation {
    ...
    async evaluate (context) {
        const mapPairs = await super.evaluate(context);
        return mapPairs((item1, item2, value1, value2, type1, type2) => {
            if (value1 === null) return item2;
            if (value2 === null) return item1;
            switch (`${type1}+${type2}`) {
...
class MulOperation extends ArithmeticOperation {
    ...
    async evaluate (context) {
        ...
        return mapPairs((item1, item2, value1, value2, type1, type2) => {
            ...
            if (type1 === "Bool") return value1 ? value2 : item2.toVoid();
            if (type2 === "Bool") return value2 ? value1 : item1.toVoid();
            switch (`${type1}*${type2}`) {
