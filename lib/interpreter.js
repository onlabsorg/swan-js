...
class ComparisonOperation extends BinaryOperation {
    ...
    _compareItems (item1, item2) {
        ...
        switch (`${item1.typeName},${item2.typeName}`) {
            ...
            default:
                // Identity check. No order defined.
                return value1 === value2 ? 0 : "ne";
        }    
    }
    ...
    _compareNamespaces (namespace1, namespace2) {
        ...
        for (let name of nameList1) {
            ...
            const cmp = this._compareTerms(term1, term2);
            if (cmp instanceof types.Undefined) return cmp;
            if (cmp !== 0) return "ne";
        }
        return 0;
    }    
...
class EqOperation extends ComparisonOperation {
    ...
    async evaluate (context) {
        ...
        return cmp instanceof types.Undefined ? cmp : new types.Bool(cmp === 0);
    }
}
...
class LtOperation extends ComparisonOperation {
    ...
    async evaluate (context) {
        const cmp = await super.evaluate(context);
        return cmp instanceof types.Undefined ? cmp : new types.Bool(cmp === -1);
    }        
}
...
class LeOperation extends ComparisonOperation {
    ...
    async evaluate (context) {
        const cmp = await super.evaluate(context);
        return cmp instanceof types.Undefined ? cmp : new types.Bool(cmp === -1 || cmp === 0);
    }        
}
