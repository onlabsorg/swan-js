...
// This functions converts any item to Boolean
...
class Undefined {
    ...
    // Returns a human-friendly description of this Undefined instance
    get description () {
        const types = this.operands.length === 1 ?
                this.constructor.stringifyOperator(this.operands[0]) :
                `(${this.operands.map(this.constructor.stringifyOperator).join(', ')})`;
        return `Undefined ${this.operation} of ${types}`;
    }
    
    // Returns a list of the descriptions of all the Undefined instances in this
    // undefined chain. For example, the operation "1 + 10 / ()" returns
    // and undefined with the following stack:
    //      [
    //          "Undefined sum of (Number, Undefined) @0:2",     // the second Undefined operand is the result of the division
    //          "Undefined division of (Number, NOTHING) @0:7"   // result of the division and second operand of the sum
    //      ]
    get stack () {
        const stack = [`${this.description} @${this.row}:${this.column}`];
        for (let operand of this.operands) {
            if (operand instanceof this.constructor) {
                return stack.concat(operand.stack);
            }
        }
        return stack;
    }
    ...
    // This function determines how an undefined operatore is stringified in 
    // the undefined instance description.
    static stringifyOperator (operator) {
        operator = Tuple(operator).normalize();
        if (operator instanceof Tuple) return operator.isEmpty() ? "NOTHING" : "Tuple";
        if (isNothing(operator)) return "NOTHING";
        if (operator instanceof Error) return String(operator);
        return detectItemType(operator);        
    }
}
...
const parse = Parser({
     ...
     errorHandler       : "$error"      // handles lexer and parser errors
});
...
const context = {
    ...
    async $apply (F, X) {
        ...
        return await Tuple(f).mapAsync(async fItem => {
            ...
            try {
                ...
                return this.undefined('resolution', error);
            }    
    ...
    // Error handler. Called on lexer or parser error.
    $error (X, Y) {
        // this should handle lexer and parser errors
    },
    
    // Undefined catching operation handler: X ?> Y
    async $catch (X, F) {
        x = await X(this);
        if (x instanceof Undefined) {
            return await this.$apply(F, () => Tuple(x.operation, ...x.operands));
        } else {
            return x;
        }
    },
    ...
    undefined (operation, ...operands) {
        const u = new Undefined(operation, ...operands);
